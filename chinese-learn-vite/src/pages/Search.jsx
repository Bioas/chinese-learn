...
export default function Search() {
  ...
  const getStatusBadge = (wordId) => {
    ...
    switch (status) {
      case 'mastered': return <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-500/20 text-green-400">Mastered</span>;
      case 'reviewing': return <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400">Reviewing</span>;
      case 'learning': return <span className="text-[10px] px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-400">Learning</span>;
      default: return <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-700/50 text-secondary">New</span>;
    }
  };
    ...
    <div className="space-y-6">
      <div className="animate-slide-up">
        <h1 className="text-3xl lg:text-4xl font-bold">
          <span className="gradient-text"><Icon name="search" /> Search</span>
        </h1>
        <p className="text-secondary mt-1">Look up any vocabulary word</p>
      </div>
      ...
      <div className="glass-card p-4 animate-slide-up">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search Chinese, pinyin, or meaning..."
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          >
            <><Icon name="search" className="mr-1" /> All Fields</>
          </button>
          <button
            onClick={() => setSearchMode('chinese')}
            className={`chip text-xs ${searchMode === 'chinese' ? 'active' : ''}`}
          >
            <><Icon name="fontFamily" className="mr-1" /> Chinese</>
          </button>
          <button
          ...
          >
            <><Icon name="bookmark" className="mr-1" /> Meaning</>
          </button>
          {query && (
      ...
      <div className="space-y-3 animate-fade-in">
        {results.length === 0 && query ? (
          <div className="glass-card p-8 text-center">
            ...
            <p className="text-secondary font-medium">No matching words found.</p>
          ...
          <div className="glass-card p-8 text-center">
            ...
            <p className="text-secondary">Start typing to search the vocabulary database</p>
            <p className="text-xs text-muted mt-1">Over {VOCABULARY.length} words available</p>
          </div>
        ) : (
          results.map((word, i) => (
            <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {word.examples[0] && (
                    <p className="text-xs text-muted mt-1">
                      {word.examples[0].chinese} - {word.examples[0].meaning}
                    </p>
                  )}
                </div>
